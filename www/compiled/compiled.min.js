...
nodeList=[];[...element.childNodes].forEach((node)=>{if(node.innerHTML!==undefined&&node.innerHTML!==null&&node.innerHTML.trim()!==""){Object.keys(data).forEach((key)=>{if(node.innerHTML.includes("{{"+key+"}}")){nodeList.push("{{"+key+"}}");}
node.innerHTML=node.innerHTML.replaceAll("{{"+key+"}}","<span data-reference='"+key+"'>"+data[key]+"</span>");});}else if(node.nodeType===Node.ELEMENT_NODE){nodeList=interpolateTemplate(node,data);console.log("nodeList1 is:",nodeList);}});nodeList.push("");nodeList=nodeList.filter(function(element){return element!==undefined;});const stringToAdd=element.tagName+">";nodeList=nodeList.map((element)=>stringToAdd+element);return nodeList;}
